...
excludeFile='/var/tmp/excludefile.txt'
...
# build list of folders to exclude for rsync to remote host
cd $sourcePath
sed "s/\r//g" $sourceFolderList | sed "/^\s*$/d" | sed "s/.*/\\/&\\/\\*\\*\\*/g" >$excludeFile

# replicate files to destination rsync server VM
cd $sourcePath
rsync --rsh="ssh" --rsync-path="rsync" --verbose --stats --temp-dir=$tmpPath -rlDvhtu --no-perms --exclude-from=$excludeFile . $destUser@$destHost:$destPath | logger -t rsync.cifs
if [[ $? != 0 ]]; then
