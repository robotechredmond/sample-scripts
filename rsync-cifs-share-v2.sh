...
cd $sourcePath
rsync --rsh="ssh" --rsync-path="rsync" --verbose --stats --temp-dir=$tmpPath -rlDvhu --no-perms --include-from=$includeFile --exclude=* . $destUser@$destHost:$destPath | logger -t rsync.cifs
if [[ $? != 0 ]]; then
